		...
		<table  align="center" border="3" class="caja2">
			<thead>
				<tr>
					...
					<td colspan="6"><h1>Listado de Jurado 3</h1></td>
					<td colspan="2"><center>
										<form action="operacion_buscar.php" metdod="POST">
											<table>
												<tr>Codigo Jurado:<br><input type="text" name=" id_jurado_3" placeholder=" solo numeros..." value=""/>
			...
			<tbody>
				...
				?> 
						<tr>
							...
							<?php 	if($row['activo']==1){?>
										<td colspan="2"><?php echo $row['ci_jur_3' ];?></td>
										...
										<td colspan="2" bgcolor="#F4FA58">	<?php echo $row['ci_jur_3' ];?></td>
										<td colspan="2" bgcolor="#F4FA58">	<?php echo $row['nom_p' ];?>
											<td colspan="1"><?php echo $row['es_p' 		];?></td>
										<td colspan="1"><?php echo utf8_encode($row['descripcion' 		]);?></td></td><?php
									}?>	
							
							<td colspan="3" ><a href="eliminar1.php?ci_p=<?php echo $row['ci_p'];?>&id_jurado_3=<?php echo $row['id_jurado_3'];?>">Eliminar</a></td>
