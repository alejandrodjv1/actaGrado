<?php
	...
	include("conexion.php");
				...
				$query="SELECT  
                       ...
                       $dia=$f[2]; $mes=$f[1]; $anio=$f[0];$ao=$f[0]-2000;
						...
						$ac = array($row['escuela'],$row['ao'],$row['semestre'],$row['id_acta']);
                                $acta  = implode("",$ac );
	...
	if($row["id_menc"]=="Honorifica"){
		$pdf->Cell(150,07,$row["mencion"],0,1,'C');
		$pdf->Cell(20);$pdf->Cell(150,01,'_____________________________________________',0,1,'C');
		}elseif($row["id_menc"]=="Publicacion"){
			$pdf->Cell(150,07,$row["mencion"],0,1,'C');
			$pdf->Cell(20);$pdf->Cell(150,01,'_____________________________________________',0,1,'C');
			}elseif($row["id_menc"]=="Honorifica y Publicacion"){
				$pdf->Cell(150,07,$row["mencion"],0,1,'C');
				$pdf->Cell(20);$pdf->Cell(150,01,'_____________________________________________',0,1,'C');
				}elseif($row["id_menc"]=="Sin mencion"){
					$pdf->SetFont('Arial','',10);
					$pdf->Cell(20);
					$pdf->MultiCell(150,07,utf8_decode('En la ciudad de Mérida a los '.$dia.' días del mes de '.$mes.' del '.$anio.','.
					' en la Sede Principal Extensión/Ampliación Mérida del Instituto Universitario Politécnico "Santiago'.
	...
	$pdf->Cell(20);$pdf->Cell(50,03,utf8_decode($row['nom_ju_1' ]),0,0,'C');
					$pdf->Cell(50,03,utf8_decode($row['nom_ju_2' ]),0,0,'C');
					$pdf->Cell(50,03,utf8_decode($row['nom_ju_3' ]),0,1,'C');
