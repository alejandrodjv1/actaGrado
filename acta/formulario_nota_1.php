                              ...
                              <tr>
                                <tr><td><label>Mencion de la Tesis :</label></td>
                                    <td><input type="text" name="id_menc" placeholder="<?php echo $row['id_menc'];?>"
                                        value="<?php echo $row['id_menc'];?>"list="id_menc"  autocomplete="off" /> 
                                        <datalist id="id_menc">
                                            <?php
                                                
                                                $query1="SELECT * FROM mencion";
                                                $res1=$conn->query($query1);
                                               
                                                $datos_usuario=$row["id_m"];
                                                while($row1=$res1->fetch_assoc()){
                                                $id=$row1['id_m'];
                                                $id_menc=$row1['id_menc'];
                                                if($dato_usuario["id_m"]==$id){
                                                echo "<option value='".utf8_encode($id_menc)."'selected='selected'>"."</option>";
                                                }
                                                else {
                                                echo "<option value='".utf8_encode($id_menc)."'>"."</option>";
                                                }
                                                }?>
