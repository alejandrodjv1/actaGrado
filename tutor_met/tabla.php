...
<body>
	...
	<center>
		...
		<table  align="center" border="3" class="caja2">
			<thead>
				<tr>
					<td colspan="1"><a href="formulario.php"> Agregar Tutor</a></a></td>
					<td colspan="6"><h1>Listado de Tutor<br>Metodologico</h1></td>
					<td colspan="6"><center>
										<form action="operacion_buscar.php" method="POST">
											<table>
												<tr>CEDULA:<br>	<input type="text" name="ced_tutor_met" placeholder=" numeros sin puntos..." value=""/>
			...
			<tbody>
				...
				<?php
					...
					$query="SELECT  *
							...
							ON tu.ced_tutor_met=pr4.ci_p
							INNER JOIN escuela AS es 
							ON tu.esc_tu_met=es.escuela
							WHERE pr4.estatus_tu_met_pr=1
							                            
							ORDER BY tu.id_tu_met ASC
							";
				...
				?> 
						<tr>
							...
							<?php 	if($row['activo']==1){?>
										...
										<td colspan="1"><?php echo $row['esc_tu_met' 		];?></td>
										<td colspan="1"><?php echo utf8_encode($row['descripcion' 		]);?></td><?php

									}elseif($row['activo']==0){?>
										<td colspan="2" bgcolor="#F4FA58">	<?php echo $row['ced_tutor_met' ];?></td>
										<td colspan="2" bgcolor="#F4FA58">	<?php echo $row['nom_tutor_met' ];?></td>
										<td colspan="1"><?php echo $row['esc_tu_met' 		];?></td>
										<td colspan="1"><?php echo utf8_encode($row['descripcion' 		]);?></td><?php
									}?>	
							
							<td colspan="3" ><a href="eliminar1.php?ci_p=<?php echo $row['ci_p'];?>&id_tu_met=<?php echo $row['id_tu_met'];?>">Eliminar</a></td>
